...
test('PUT /users/:id route', async (t) => {
  const user = {
  ...
  t.test('should update the user and return 200', async (t) => {
    const body = {
  ...
  t.test('should not update the user because password and email are missing in the body', async (t) => {
    const body = {
    };
    ...
    const parsedResponse = response.json();
    t.same(parsedResponse.message, "body should have required property 'email', body should have required property 'password', body should have required property 'email', body should match some schema in anyOf");
