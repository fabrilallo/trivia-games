...
export default async function createQuiz (fastify, _opts) {
  const { httpErrors, getDuplicatedItems, isCorrectValueUnique, createQuiz } =
    fastify;
  ...
  async function createQuizHandler (req, reply) {
    ...
    if (duplicatedQuestions.length !== 0) {
      ...
      throw httpErrors.badRequest(
        `The following questions are duplicated: '${invalidQuestionNames}'`
      );
    }
    ...
    if (invalidIsCorrectQuests.length !== 0) {
      ...
      throw httpErrors.badRequest(
        `The following questions have more than one correct answer: '${invalidQuestionNames}'`
      );
    }
    ...
    if (questWithDuplicatedAnswers.length !== 0) {
      ...
      throw httpErrors.badRequest(
        `The following questions have same answers with different 'isCorrect' values: '${invalidQuestionNames}'`
      );
    }

    const newQuiz = await createQuiz({ quiz: req.body, userId: user.id });
